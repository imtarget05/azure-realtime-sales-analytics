...
$DRIFT_TRIGGER_MODE = if ($env:DRIFT_TRIGGER_MODE) { $env:DRIFT_TRIGGER_MODE } else { "azureml" }
$DRIFT_MAE_ABS_THRESHOLD = if ($env:DRIFT_MAE_ABS_THRESHOLD) { $env:DRIFT_MAE_ABS_THRESHOLD } else { "25" }
$DRIFT_MONITOR_WINDOW_HOURS = if ($env:DRIFT_MONITOR_WINDOW_HOURS) { $env:DRIFT_MONITOR_WINDOW_HOURS } else { "24" }
$DRIFT_MONITOR_MIN_SAMPLES = if ($env:DRIFT_MONITOR_MIN_SAMPLES) { $env:DRIFT_MONITOR_MIN_SAMPLES } else { "24" }
$DRIFT_MONITOR_COOLDOWN_MINUTES = if ($env:DRIFT_MONITOR_COOLDOWN_MINUTES) { $env:DRIFT_MONITOR_COOLDOWN_MINUTES } else { "120" }
$DRIFT_NOTIFY_FAILURE_FALLBACK_LOCAL = if ($env:DRIFT_NOTIFY_FAILURE_FALLBACK_LOCAL) { $env:DRIFT_NOTIFY_FAILURE_FALLBACK_LOCAL } else { "true" }
...
$functionAppSettings = @(
  ...
  "AML_WORKSPACE_NAME=$AML_WORKSPACE",
  "AML_RESOURCE_GROUP=$RESOURCE_GROUP",
  "AML_SUBSCRIPTION_ID=$SUBSCRIPTION_ID",
  "DRIFT_TRIGGER_MODE=$DRIFT_TRIGGER_MODE",
  "DRIFT_MAE_ABS_THRESHOLD=$DRIFT_MAE_ABS_THRESHOLD",
  "DRIFT_MONITOR_WINDOW_HOURS=$DRIFT_MONITOR_WINDOW_HOURS",
  "DRIFT_MONITOR_MIN_SAMPLES=$DRIFT_MONITOR_MIN_SAMPLES",
  "DRIFT_MONITOR_COOLDOWN_MINUTES=$DRIFT_MONITOR_COOLDOWN_MINUTES",
  "DRIFT_NOTIFY_FAILURE_FALLBACK_LOCAL=$DRIFT_NOTIFY_FAILURE_FALLBACK_LOCAL"
)

if ($SLACK_WEBHOOK_URL) {
  $functionAppSettings += "SLACK_WEBHOOK_URL=$SLACK_WEBHOOK_URL"
}
