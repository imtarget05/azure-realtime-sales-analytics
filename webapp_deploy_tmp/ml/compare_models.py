...
def generate_training_data(n_samples: int = 50000) -> pd.DataFrame:
    """Sinh dữ liệu sales giả lập cho training — aligned với train_model.py."""
    np.random.seed(42)

    store_ids = np.random.choice(["S01", "S02", "S03"], n_samples)
    product_ids = np.random.choice(["COKE", "PEPSI", "BREAD", "MILK"], n_samples)
    category_map = {"COKE": "Beverage", "PEPSI": "Beverage", "BREAD": "Bakery", "MILK": "Dairy"}
    categories = np.array([category_map[p] for p in product_ids])
    price_map = {"COKE": 1.5, "PEPSI": 1.4, "BREAD": 1.15, "MILK": 1.6}
    ...
    is_rainy = np.random.choice([0, 1], n_samples, p=[0.7, 0.3])
    holiday = np.random.choice([0, 1], n_samples, p=[0.9, 0.1])

    # Label encode
    store_enc = np.array([{"S01": 0, "S02": 1, "S03": 2}[s] for s in store_ids])
    product_enc = np.array([{"COKE": 0, "PEPSI": 1, "BREAD": 2, "MILK": 3}[p] for p in product_ids])
    category_enc = np.array([{"Beverage": 0, "Bakery": 1, "Dairy": 2}[c] for c in categories])

    base_prices = np.array([price_map[p] for p in product_ids])
    quantity = np.random.randint(1, 11, n_samples)

    # Revenue with realistic patterns
    hour_effect = np.sin(hours * np.pi / 12) * 2 + 1
    weekend_effect = is_weekend * 1.5
    month_effect = np.sin((months - 1) * np.pi / 6) * 1.0
    weather_effect = -is_rainy * 0.5
    holiday_effect = holiday * 2.0
    noise = np.random.normal(0, 0.5, n_samples)

    revenue = (
        base_prices * quantity
        * (1 + hour_effect * 0.1 + weekend_effect * 0.05 + month_effect * 0.05)
        + weather_effect + holiday_effect + noise
    )
