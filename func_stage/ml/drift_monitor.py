...
def _send_retrain_notifications(report: dict) -> dict:
    try:
        from monitoring.alerts import send_drift_alert

        return send_drift_alert(report)
    except Exception as exc:
        return {
            "success": False,
            "error": str(exc),
            "status": "notification_exception",
        }


def _slack_notification_succeeded(alert_result: dict) -> bool:
    if not isinstance(alert_result, dict):
        return False

    slack_result = alert_result.get("slack")
    if isinstance(slack_result, dict):
        return bool(slack_result.get("success"))

    return False


def _run_local_retrain_fallback(trigger_result: dict) -> dict:
    fallback_result = trigger_retrain(promote=True)
    trigger_result["local_notify_fallback"] = fallback_result
    trigger_result["notify_fallback_reason"] = "slack_notification_failed"
...
def run_monitor(
    ...
    try:
        ...
        if should_trigger:
            ...
            alert_payload = _build_trigger_report(
            ...
            alert_result = _send_retrain_notifications(alert_payload)
            trigger_result["alerts"] = alert_result

            fallback_on_notify_failure = os.getenv(
                "DRIFT_NOTIFY_FAILURE_FALLBACK_LOCAL", "true"
            ).lower() in ("1", "true", "yes")
            local_already_ran = any(
                key in trigger_result for key in ("local", "local_notify_fallback")
            )

            if (
                fallback_on_notify_failure
                and not _slack_notification_succeeded(alert_result)
                and not local_already_ran
            ):
                print("[WARN] Slack notification failed; running local retrain fallback.")
                fallback_result = _run_local_retrain_fallback(trigger_result)
